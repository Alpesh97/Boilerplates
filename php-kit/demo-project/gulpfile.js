...
const files = {
	...
	scssInnerPath: 'assets/scss/**/*.scss',
	jsPath: 'assets/js/*.js',
...
function scssTask() {
    ...
    function innerscssTask() {
    	return src(files.scssInnerPath)
        .pipe(plumber())  // For Error Handler And Automatic Gulp Restart
    }    
// For Js
function jsTask() {
...
function watchTask() {
	watch([files.scssPath, files.jsPath, files.vendorJS,files.fonts,files.images,files.scssInnerPath],
		series(
			innerscssTask,
			vendorjsTask,
			imageTask,
